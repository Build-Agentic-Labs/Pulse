...
// An explode sequence is at most a few hundred frames; anything outside this range is bogus.
const MAX_FRAME_NUMBER = 100_000;
...
function boundedFrameNumber(value: unknown): number | undefined {
  return typeof value === "number" && Number.isFinite(value) && value >= 0
    ? Math.min(Math.trunc(value), MAX_FRAME_NUMBER)
    : undefined;
}
    ...
    const explodedView = await saveExplodedViewToSupabase(
      {
        ...
        explodeConfigName: boundedMetaText(meta.explodeConfigName),
        frameNumber: boundedFrameNumber(meta.frameNumber),

...
- **DB/integration tests (spec §10, pgTAP)** are **written** in `supabase/tests/sops_enforcement_test.sql` — covering born-effective INSERT rejection, content-freeze on a non-draft document-only PATCH, SoD (submitter can't approve; `approved_by` DB-stamped), DEPT-TYPE-NNN numbering, and one-effective-via-`effective_revision_id`. Run with `supabase db reset && supabase test db`. **Not executed in this environment** (Docker daemon unavailable), so treat as *pending-green* until run against a local/CI Postgres before merge.

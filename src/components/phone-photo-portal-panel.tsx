...
function fallbackPortalUrl(projectId?: string) {
  ...
  return `${window.location.origin}${projectId ? `/projects/${projectId}/mobile-photos` : "/mobile-photos"}`;
}
...
export function PhonePhotoPortalPanel({ project }: { project?: PlannerProjectContext }) {
  ...
  useEffect(() => {
    ...
    fetch(phonePortalApiUrl, { cache: "no-store" })
      ...
      .then((payload) => {
        ...
        setPhonePortalUrl(payload.url ?? fallbackPortalUrl(project?.projectId));
      })
      .catch(() => {
        if (mounted) {
          setPhonePortalUrl(fallbackPortalUrl(project?.projectId));
        }
      });
  ...
  return (
    <section className="ui-settings-section">
      ...
      <p className="ui-settings-section-desc">Scan the QR code on a phone to capture step photos in the procedure editor.</p>
        ...
        <div className="grid gap-4 md:grid-cols-[200px_minmax(0,1fr)] md:items-start">
          ...
          <div className="space-y-3">
            <label className="block">
              <span className="ui-field-label">Portal URL</span>
              <input
            ...
            <a href={phonePortalUrl || "/mobile-photos"} className="ui-btn-secondary inline-flex h-8 px-3 text-[10px]">
              Open portal
            </a>
          </div>
        </div>

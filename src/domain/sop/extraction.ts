...
- For a decision, populate \`decisionBranches\` when the source makes the paths clear. Use the destination activity's 1-based \`step\` for \`yesTargetStep\` and \`noTargetStep\`; use null when that outcome ends the process. Yes and No may share a destination when the source requires the same follow-up for both outcomes (for example, communicating an approval or rejection). Preserve that shared follow-up; never end a branch merely to make the destinations different. If one path is unclear, omit it for the author to complete rather than guessing. Do not add \`decisionBranches\` to non-decision activities.

...
- For a decision, populate \`decisionBranches\` when the source makes the paths clear. Use the destination activity's 1-based \`step\` for \`yesTargetStep\` and \`noTargetStep\`; use null when that outcome ends the process. Yes and No must not point to the same destination — if one path is unclear, omit it for the author to complete rather than guessing. Do not add \`decisionBranches\` to non-decision activities.

...
- The Procedure may contain a RASIC responsibility matrix. If you can identify activities and the roles responsible, populate \`procedure.roles\` with the column role names and give each activity an \`assignments\` map of role-name -> one of R, A, S, I, C. If there is no matrix, leave roles/assignments empty and capture the steps as activity descriptions in order. Assign RASIC by who actually acts on THAT specific step — do not copy every role onto every step. Most steps have a single Responsible plus only the Approver / Support / Informed / Collaborator that genuinely apply. Place Approval (A) ONLY on the step where the approval is actually granted (typically the decision / approval step); a later step that merely acts on or communicates an already-made decision uses R / S / I, not A.

-- SOP transition guard v3 — supersedes v2. The database remains the ONLY enforcement layer,
-- so every rule here must survive a raw PostgREST INSERT/PATCH from any org-tool editor.
--
-- Changes from v2:
--   * Gate A: the roster must be complete (>=1 responsible, exactly 1 accountable, every seat's
--     signer a strict member of that seat's department), the submitter may hold no blocking
--     seat, authorship must be signed against the current content, and no objection may be open.
--   * Gate B: quorum, not a single approver. approved_by is stamped with whoever's signature
--     completed it (the caller of the auto-advance inside sign_sop).
--   * Gate C: Quality must not hold a seat, must not be the author or submitter, and must not
--     be the person who overruled an objection this cycle.
--   * Gate D: a revision needs a reason, and bumps review_cycle.
--   * Reject vs Recall: a rejection is driven from inside sign_sop and carries its reason on the
--     signature. The submitter may recall with no reason and no signature.
--   * INSERT strips the version columns too (v2 did not, so a raw insert could forge a version
--     and make a FIRST release look like a revision).

-- pgTAP tests for the workspace RASIC role vocabulary (sop_rasic_roles).
--
-- The rule in one sentence: anyone who can edit SOPs in the workspace may read and add roles;
-- only workspace owners/admins may rename or delete one. Curation is a narrower privilege than
-- authoring on purpose — an author adding a role is routine, rewriting the shared vocabulary is
...
insert into public.workspaces (id, name) values ('ws_rr', 'Roles Org');
insert into public.workspace_auto_join_domains (domain, workspace_id) values ('test.dev', 'ws_rr');

insert into auth.users (id, aud, role, email)
values
  ('f0000000-0000-0000-0000-000000000001', 'authenticated', 'authenticated', 'rr-author@test.dev'),
  ('f0000000-0000-0000-0000-000000000002', 'authenticated', 'authenticated', 'rr-viewer@test.dev'),
  ('f0000000-0000-0000-0000-000000000003', 'authenticated', 'authenticated', 'rr-admin@test.dev'),
  ('f0000000-0000-0000-0000-000000000004', 'authenticated', 'authenticated', 'rr-outsider@test.dev');

insert into public.workspace_members (workspace_id, user_id, role) values
  ('ws_rr', 'f0000000-0000-0000-0000-000000000001', 'editor'),
  ('ws_rr', 'f0000000-0000-0000-0000-000000000002', 'viewer'),
...
select is(
  (select count(*) from public.sop_rasic_roles where workspace_id = 'ws_rr'),
  1::bigint,
  'an org-tool editor reads the workspace vocabulary'
);
select lives_ok(
  $$ insert into public.sop_rasic_roles (workspace_id, name) values ('ws_rr', 'Shift Coordinator') $$,
  'an org-tool editor may add a role'
);
...
-- ---------------------------------------------------------------------------
update public.sop_rasic_roles set name = 'Renamed By Author' where id = 'rr_seed';
select is(
  (select name from public.sop_rasic_roles where id = 'rr_seed'),
  'Line Auditor',
  'an author''s UPDATE matches no row: renaming is owner/admin only'
);
delete from public.sop_rasic_roles where id = 'rr_seed';
select is(
  (select count(*) from public.sop_rasic_roles where id = 'rr_seed'),
  1::bigint,
...
select throws_ok(
  $$ insert into public.sop_rasic_roles (workspace_id, name) values ('ws_rr', 'Sneaky') $$,
  null,
  'an org-tool viewer cannot add a role'
);
...
update public.sop_rasic_roles set name = 'Line Auditor (Nights)' where id = 'rr_seed';
select is(
  (select name from public.sop_rasic_roles where id = 'rr_seed'),
  'Line Auditor (Nights)',
  'a workspace admin can rename a role'
);
delete from public.sop_rasic_roles where id = 'rr_seed';
select is(
  (select count(*) from public.sop_rasic_roles where id = 'rr_seed'),
  0::bigint,
  'a workspace admin can remove a role'
);
...
select is(
  (select count(*) from public.sop_rasic_roles where workspace_id = 'ws_rr'),
  0::bigint,
  'a user outside the workspace reads none of its roles'
);
